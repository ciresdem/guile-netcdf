;;; Help --- Show help on guile-netcdf commands
...
;; Usage: help
;;
;; Show help for netcdf scripts.

;;; Code:

(define-module (netcdf scripts help)
...
(define %help "
Show help on netcdf commands.  With --all, show arcane incantations as
...
Usage: ncguile COMMAND [ARGS]
Run command-line scripts provided by GUILE-NETCDF and related programs.

Commands:
" (list-ref %title-list (modulo (random (current-time)) (length %title-list))))

  (for-each
   (lambda (name)
     (let* ((modname `(netcdf scripts ,(string->symbol name)))
            (mod (resolve-module modname #:ensure #f))
            (summary (and mod (and=> (module-variable mod '%summary)
                                     variable-ref))))
       (if (and mod
                (or all?
                    (let ((v (module-variable mod '%include-in-netcdf-list)))
                      (if v (variable-ref v) #t))))
           (if summary
               (format #t "  ~A ~23t~a\n" name summary)
               (format #t "  ~A\n" name)))))
   (find-submodules '(netcdf scripts)))
  (format #t "
For help on a specific command, try \"ncguile help COMMAND\".
                 ...
                 (let ((var (module-variable mod '%synopsis)))
                   (if var
                       ...
                       (string-append (module-command-name mod)
                                      " OPTION...")))
                 #\newline)))
    (display "Usage: ncguile " port)
    (display (car usages))
    (newline port)
    (for-each (lambda (u)
                (display "       ncguile " port)
                (display u port)
                (newline port))
              (cdr usages))))
...
(define (main . args)
  (cond
   ...
   ((and (null? (cdr args)) (not (string-prefix? "-" (car args))))
    ...
    (let ((name (car args)))
      (cond
       ((resolve-module `(netcdf scripts ,(string->symbol name)) #:ensure #f)
        => (lambda (mod)
             (show-help mod)
             (exit 0)))
       (else
